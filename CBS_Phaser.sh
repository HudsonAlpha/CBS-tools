...
print_usage() {
	...
	echo "			If results were inconclusive, Phaser can map both sets of sex-specific k-mers by omitting this option (DEFAULT)."
	echo
	...
	echo "	-o  Output directory path and name, DEFAULT: ./CBS_Phaser_out"
	echo
	echo "	-r	Redo/overwrite results in output directory"
	echo "			CBS Phaser's default behavior won't overwrite files of the same name in output directory."
	echo "			If additional k-mer lists are added to an existing Phaser run, it will skip existing outputs and only run on new inputs." 
...
do
	...
	if [ -s "${assem}.amb" ] && [ -s "${assem}.ann" ] && [ -s "${assem}.bwt" ] && [ -s "${assem}.pac" ] && [ -s "${assem}.sa" ] && [ "$redo" = 0 ]; then
		...
		echo "Existing BWA index files found for ${assem}! WARNING: redo (-r) set - CBS Phaser will overwrite these files!"
		bwa index ${assem}
	else
		bwa index ${assem}
	fi	
	...
	do
		if [ -s "kmer_lists/${cf}.fasta" ]; then
			...
			if [ -s "${output}/${hap}/${cf}_scaffolds.bam" ] && [ "$redo" = 0 ]; then
				echo "Existing k-mer mapping depth file found! Looks like Phaser successfully finished mapping this k-mer list, moving on to next input"
				echo "Use -r to redo k-mer mapping"
			elif [ -s "${output}/${hap}/${cf}_scaffolds.bam" ] && [ "$redo" = 1 ]; then
				echo "Existing k-mer mapping depth file found! WARNING: redo (-r) set - CBS Phaser will overwrite mapping output for this k-mer list!"
	...
	if [ "$histogram_plots" = 1 ]; then
		...
		echo
		Rscript "${SCRIPT_DIR}"/src/Phaser_kmer_plotting.R -r "${output}/${hap}.chromSizes" -p "${output}/${hap}" -o "${output}/${plot_prefix}.${hap}" -b "${bin_size}" -m ${min_size}
	fi
done
